        ...
        std::thread::spawn(move || {
            loop {
                ...
                if !producer.is_full() {
                    ...
                    if !Self::decode_next_packet(&mut *format, &mut *decoder, track_id, &mut producer, &state) {
        ...
        C: Consumer<Item = i32> + Send + 'static,
    {
        let stream = self.device.build_output_stream(
            ...
            move |data: &mut [T], _: &cpal::OutputCallbackInfo| {
                ...
                // 检查是否播放结束：解码已结束 且 缓冲区这次没读出数据（意味着空了）
                if read_count == 0 && state.decoder_done.load(Ordering::SeqCst) {
                    if !state.is_finished.swap(true, Ordering::SeqCst) {
                        state.finish_notify.notify_waiters();
                    }
                }

                state.current_frame.fetch_add((read_count / channels) as u64, Ordering::Relaxed);
            },
