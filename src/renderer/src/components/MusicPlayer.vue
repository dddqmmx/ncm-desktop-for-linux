        ...
        <div class="section-center">
          <div class="playback-controls">
            <button class="icon-btn sm shuffle">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M16 3h5v5M4 20L21 3M21 16v5h-5M15 15l-5 5M4 4l5 5"/></svg>
            </button>
...
.player-container-position {
  ...
  width: 100%;
  display: flex;
  flex-direction: column; /* 垂直排列列表和播放条 */
  align-items: center;    /* 水平居中 */
  z-index: 100;
  pointer-events: none;   /* 允许点击到底层内容 */
}
...
.track-artist { font-size: 11px; color: rgba(0,0,0,0.5); }
...
.inner-glow.active { opacity: 0.15; }

.progress-area { width: 100%; padding: 4px 0; }
.progress-track {
  width: 100%; height: 4px;
  background: rgba(0, 0, 0, 0.05);
  border-radius: 2px; position: relative;
}
.progress-bar { height: 100%; background: #111; border-radius: 2px; }
.hidden-range {
  position: absolute;
  top: -10px; left: 0;
...
.section-right { display: flex; justify-content: flex-end; gap: 8px; }
...
.icon-btn.active { background: rgba(0,0,0,0.1); color: #000; }
.icon-btn.sm { opacity: 0.4; }

@media (max-width: 650px) {
  .section-left, .section-right { min-width: auto; }
  .track-metadata, .sm { display: none; }
  .playlist-popup { width: calc(100vw - 40px); }
}
</style>
